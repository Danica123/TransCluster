...
########################################
feature = []  # 列表list类型
expression = pd.read_csv('../data/Adipose/human_Adipose1372.csv',low_memory=False)#方便获取csv中表头对应的值，但忽略line_0
print('###################expression.shape:',expression.shape)
file = open('../data/Adipose/human_Adipose1372.csv')  # 读取训练集文件
...
##################################
testlabel = []
file = open('../data/lung/human_Lung6338_celltype.csv')#读取训练集文件
lable_lines = file.readlines()
lable_line_0 = lable_lines[0].strip('\n').split(',')
file.close()
#转int
...
##################################
feature = []  # 列表list类型
testexpression = pd.read_csv('../data/lung/human_Lung6338_data.csv',low_memory=False)#方便获取csv中表头对应的值，但忽略line_0
print('testexpression.shape:',testexpression.shape)#(13,1286)
file = open('../data/lung/human_Lung6338_data.csv')  # 读取训练集文件
lines = file.readlines() #读行
line_0 = lines[0].strip('\n').split(',') #line.strip('\n')移除换行符并返回列表,split(',')通过指定分隔符,对字符串进行切片
#得到特征
for i in range(1,len(line_0)):
...
import csv
with open('../data/lung/lungtrain.csv','w',newline='') as csvtrain:
