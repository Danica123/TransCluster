...
########################################
feature = []  # 列表list类型
expression = pd.read_csv('../data/kidney/comparison/testdata.csv',low_memory=False)#方便获取csv中表头对应的值，但忽略line_0
print('###################expression.shape:',expression.shape)
file = open('../data/kidney/comparison/testdata.csv')  # 读取训练集文件
...
#############################################################
label = []
file = open('../data/kidney/comparison/testcelltype.csv')#读取训练集type文件
...
import csv
with open('../data/liver/trainfloatnew.csv','w',newline='') as csvtrain:
