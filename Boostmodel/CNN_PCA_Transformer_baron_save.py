...
expression = pd.read_csv('../data/Baron/new/train_small.csv')#方便获取csv中表头对应的值，但忽略line_0
print(expression.shape)
file = open('../data/Baron/new/train_small.csv')  # 读取训练集文件
...
feature_train = list(array_feature)

#############################################################
# name_label = []
# for i in range(7,13):#13
#         name_label.append('expression-'+str(i)+'label.csv')
# print(name_label)
label = []
# for i in name_label:
file = open('../data/Baron/trainlabel.csv')#读取训练集文件
...
# name_test= []
# for i in range(1,5):
#         name_test.append('test'+str(i)+'.csv')
# print(name_test)
feature = []  # 列表list类型
# for i in name_test:
testexpression = pd.read_csv('../data/Baron/new/test_small.csv')#方便获取csv中表头对应的值，但忽略line_0
print(testexpression.shape)
file = open('../data/Baron/new/test_small.csv')  # 读取训练集文件
lines = file.readlines() #读行
line_0 = lines[0].strip('\n').split(',') #line.strip('\n')移除换行符并返回列表,split(',')通过指定分隔符,对字符串进行切片
#得到特征
for i in range(1,len(line_0)):
...
file.close()
array_feature = np.array(feature)
feature_test = list(array_feature)
#################################
#pca = PCA(n_components=4215)#保持降维后的数据保持90%的信息，即指定降维后的 方差和 占比，比例越大，降维后保留的信息越多，会自动确认降维的维度。
pca = PCA(n_components=13)
feature_train = pca.fit_transform(feature_train)
feature_test = pca.transform(feature_test)
feature_train = list(feature_train)
...
final_output = Dense(14,'softmax',**params_dict)(final_output)
###########
...
with open('../modelsave/cnn_pca_transformer_baron_epoch20.txt','w',newline='') as f:
