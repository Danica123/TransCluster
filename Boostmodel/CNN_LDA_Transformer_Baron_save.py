...
########################################
feature = []  # 列表list类型
expression = pd.read_csv('../data/Baron/new/ldak/train1.csv')#方便获取csv中表头对应的值，但忽略line_0
print('###################expression.shape:',expression.shape)
file = open('../data/Baron/new/ldak/train1.csv')  # 读取训练集文件
...
##################################
feature = []  # 列表list类型
testexpression = pd.read_csv('../data/Baron/new/ldak/test1.csv')#方便获取csv中表头对应的值，但忽略line_0
print('testexpression.shape:',testexpression.shape)#(13,1286)
file = open('../data/Baron/new/ldak/test1.csv')  # 读取训练集文件
lines = file.readlines() #读行
line_0 = lines[0].strip('\n').split(',') #line.strip('\n')移除换行符并返回列表,split(',')通过指定分隔符,对字符串进行切片
#得到特征
for i in range(1,len(line_0)):
...
with open('../data/Baron/new/ldak/1.txt','w',newline='') as f:
